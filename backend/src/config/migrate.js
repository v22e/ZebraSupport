...
const MIGRATIONS = [
  ...
  `
    ...
    BEGIN
      IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'plan_tier') THEN
        ALTER TYPE plan_tier ADD VALUE IF NOT EXISTS 'standard';
      END IF;
    END $$;
  `,
  `
  ...
  `
    ...
    BEGIN
      IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'organisations') THEN
        UPDATE organisations SET plan = 'standard'::plan_tier WHERE plan::text IN ('free', 'plus');
      END IF;
    END $$;
  `,
  async (client) => {
    ...
    if (exists.rowCount === 0) {
      await client.query(
        `INSERT INTO users (org_id, name, email, password_hash, role, active)
         VALUES (NULL, 'Zebra Platform Admin', $1, crypt($2, gen_salt('bf')), 'superadmin', true)`,
        [email, password]
      );
    }
  },
